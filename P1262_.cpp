//  Luogu contest  P1262 间谍网络
//  Created at 01.02.2026 21:07:33 in file P1262_.cpp
//  By zfq
#include <bits/stdc++.h>
// #include <bits/extc++.h>
#define LIMIT 22
...
void solve(){
    ...
    auto sccG = scc.build_scc_graph();
    //获取缩点后的权重
    vector<int> scc_weights(scc.scc_cnt,inf);
    for (int i = 0; i < n; i++) {
        scc_weights[scc.scc_id[i]] = min(weights[i], scc_weights[scc.scc_id[i]]);
    }
    ...
    if (flag) {
        cout << "NO" << endl;
        int mn_id = inf;
        //拓扑排序，出发点只能是入度为0，并且可访问的，标记一下。
        queue<int> q;
        vector<int> vis(scc_in.size(),false);
        for (int i = 0; i < scc.scc_cnt; i++) {
            if (scc_weights[i] < inf) {
                q.push(i);
                vis[i] = true;
            }
        }
