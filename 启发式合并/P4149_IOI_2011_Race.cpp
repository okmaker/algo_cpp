...
//  Luogu contest  P4149 [IOI 2011] Race
//  Created at 10.01.2026 11:32:45 in file P4149_IOI_2011_Race.cpp
//  By zfq
#include <bits/stdc++.h>
// #include <bits/extc++.h>
#define LIMIT 22
...
void solve() {
    ...
    int res = inf;
    auto add = [&](int u){
        int d = dsu.depth[u],dis = dsu.dist[u];
        if (!min_dep.contains(dis)) min_dep[dis] = inf;
        min_dep[dis] = min(min_dep[dis],d);
    };
    auto del = [&](int u){
        int dis = dsu.dist[u];
        min_dep.erase(dis);
    ...
    auto calc = [&](int u,int root) {
        // dist[u] + dist[v] - 2 dist[root] = k
        int target = k - dsu.dist[u] + 2 * dsu.dist[root];
        if (min_dep.contains(target)) res = min(res,min_dep[target] + dsu.depth[u] - 2 * dsu.depth[root]);
    };
