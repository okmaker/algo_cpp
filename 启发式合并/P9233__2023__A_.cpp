//  Luogu contest  P9233 [蓝桥杯 2023 省 A] 颜色平衡树
//  Created at 09.01.2026 13:20:05 in file P9233__2023__A_.cpp
//  By zfq
#include <bits/stdc++.h>
// #include <bits/extc++.h>
#define LIMIT 22
...
void solve(){
    ...
    for (int i = 0; i < n; i++) {
        ...
        colors[i] = c;
        mx = max(mx,c);
        if (f != -1) g[f].push_back(i);
    }
    DSUOnTree dsu(n,g);
    dsu.prepare(0,-1);
    vector<int> cnt(mx + 1),cntcnt(n + 1);
    int res = 0;
    auto add = [&](int u) {
        int c = colors[u];
        if (cnt[c] > 0) cntcnt[cnt[colors[u]]]--;
        cnt[colors[u]]++;
        cntcnt[cnt[c]]++;
    };
    auto del = [&](int u) {
        ...
        cntcnt[cnt[c]]--;
        cnt[c]--;
        if (cnt[c] > 0) cntcnt[cnt[c]]++;
    };
    auto getAns = [&](int u) {
        int k = cnt[colors[u]];
        if (cntcnt[k] * k == dsu.siz[u]) res++;
    };
