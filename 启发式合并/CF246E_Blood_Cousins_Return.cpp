//  Luogu contest  CF246E Blood Cousins Return
//  Created at 09.01.2026 14:24:10 in file CF246E_Blood_Cousins_Return.cpp
//  By zfq
#include <bits/stdc++.h>
// #include <bits/extc++.h>
#define LIMIT 22
#define inf 0x3f3f3f3f3f3f
#define endl '\n'

#define int long long
using namespace std;
//using namespace __gnu_pbds;
//using ordered_set = tree<int, null_type, less<>, rb_tree_tag, tree_order_statistics_node_update>;

using pii = pair<int,int>;
using v2d = vector<vector<int>>;
const double eps = 1e-7;
int dx[] =  {0,0,1,-1};
int dy[] = {1,-1,0,0};
const int MOD = 1e9 + 7;
const int N = 1e5 + 5;
const int M = 1e5 + 7;
/*
    
*/
template <typename Graph>
class DSUOnTree {
public:
    int n;
    const Graph& g;
    std::vector<int> siz, son,depth;


    DSUOnTree(int n, const Graph& g) : n(n), g(g), siz(n), son(n, -1),depth(n,0) {}

    // 第一遍 DFS：预处理 siz 和 son
    void prepare(int u, int fa) {
        siz[u] = 1;
        son[u] = -1;
        if (fa != -1) depth[u] = depth[fa] + 1;
        for (auto v : g[u]) {
            if (v == fa) continue;
            prepare(v, u);
            siz[u] += siz[v];
            if (son[u] == -1 || siz[v] > siz[son[u]]) son[u] = v;
        }
    }

    // 核心 DFS 逻辑：利用模板参数 Add, Del, Ans 实现零开销回调
    template <typename Add, typename Del, typename Ans>
    void solve(int u, int fa, bool keep, Add&& add, Del&& del, Ans&& ans) {
        // 1. 递归处理轻儿子
        for (auto v : g[u]) {
            if (v == fa || v == son[u]) continue;
            solve(v, u, false, add, del, ans);
        }

        // 2. 递归处理重儿子
        if (son[u] != -1) {
            solve(son[u], u, true, add, del, ans);
        }

        // 3. 暴力加入轻儿子及其子树
        for (auto v : g[u]) {
            if (v == fa || v == son[u]) continue;
            update_recursive(v, u, add);
        }
        add(u); // 加入当前节点

        // 4. 记录答案
        ans(u);

        // 5. 是否清除
        if (!keep) {
            update_recursive(u, fa, del);
        }
    }

private:
    // 辅助递归函数
    template <typename Updater>
    void update_recursive(int u, int fa, Updater&& updater) {
        updater(u);
        for (auto v : g[u]) {
            if (v == fa) continue;
            update_recursive(v, u, updater);
        }
    }
};
/*
    可以让0节点作为根节点，这样让森林变为一棵树
 */
void solve(){
    int n; cin >> n;
    vector<vector<int>> g(n + 1);
    vector<string> names(n + 1);
    for (int i = 1; i <= n; i++) {
        string s;int r; cin >> s >> r;
        names[i] = s;
        g[r].push_back(i);
    }
    int m; cin >> m;
    vector<vector<array<int,2>>> qs(n + 1);//(层数，问题id)
    for (int i = 0; i < m; i++) {
        int v,k; cin >> v >> k;
        qs[v].push_back({k,i});
    }
    DSUOnTree dsu(n + 1, g);
    dsu.prepare(0, -1);
    vector<int> res(m);
    //使用set维护每一层的信息
    vector<set<string>> layer(n + 1);
    auto add = [&](int u) {
        int d = dsu.depth[u];
        layer[d].insert(names[u]);
    };
    auto del = [&](int u) {
        int d = dsu.depth[u];
        layer[d].clear();
    };
    auto getAns = [&](int u) {
        int d = dsu.depth[u];
        for (auto [k,id] : qs[u]) {
            int cnt = d + k <= n ? layer[d + k].size() : 0;
            res[id] = cnt;
        }
    };
    dsu.solve(0,-1,false,add,del,getAns);
    for (int i = 0; i < m; i++) {
        cout << res[i] << endl;
    }
}
signed main() {
    ios::sync_with_stdio(false); cin.tie(0); cout.tie(0);
    int T = 1;//cin >> T;
    while(T--){
        solve();
    }
    
    return 0;
}