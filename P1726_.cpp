//  Luogu contest  P1726 上白泽慧音
//  Created at 01.02.2026 17:21:19 in file P1726_.cpp
//  By zfq
#include <bits/stdc++.h>
// #include <bits/extc++.h>
#define LIMIT 22
...
void solve() {
    ...
    scc.run();
    vector<vector<int>> sccG = scc.build_scc_graph();
    int scc_cnt = scc.scc_cnt;
    for (int i = 0; i < scc_cnt; i++) sort(sccG[i].begin(), sccG[i].end());
    int id = 0;
    for (int i = 1; i < scc_cnt; i++) {
        if (sccG[i].size() > sccG[id].size()) {
            id = i;
        }else if (sccG[i].size() == sccG[id].size() && sccG[i][0] < sccG[id][0]) {
            id = i;
        }
    }
    cout << sccG[id].size() << endl;
    for (auto v: sccG[id]) cout << v << " ";
    cout << endl;
}
